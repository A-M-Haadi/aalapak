            ...
            <div class="hidden sm:flex sm:items-center sm:ml-6">
                    ...
                    <x-dropdown align="right" width="48">
                                ...
                                <div class="flex items-center space-x-3">
                                    ...
                                    <div class="text-left">
                                        ...
                                        <div class="text-xs text-gray-500">
                                            @if(Auth::user()->role == 'buyer') 👤 Buyer
                                            @elseif(Auth::user()->role == 'seller') 🏪 Seller
                                            @else 👑 Admin @endif
                                        </div>
                                    </div>
                        ...
                        <x-slot name="content">
                            ...
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <x-dropdown-link :href="route('logout')" onclick="event.preventDefault(); this.closest('form').submit();" class="flex items-center space-x-2 text-red-600 hover:bg-red-50">
                ...
                @else
                    <div class="flex items-center space-x-4">
                        <a href="{{ route('login') }}" class="text-gray-600 hover:text-blue-600 font-medium transition-colors text-sm">Masuk</a>
                        <a href="{{ route('register') }}" class="px-5 py-2.5 rounded-xl bg-gradient-to-r from-blue-600 to-purple-600 text-white font-medium hover:shadow-lg transform hover:-translate-y-0.5 transition-all text-sm">Daftar Sekarang</a>
                    </div>
                @endauth
            </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1 bg-white border-t border-gray-200">
            ...
            @if(Auth::check() && Auth::user()->role == 'admin')
                ...
                <x-responsive-nav-link :href="route('admin.sellers.index')" :active="request()->routeIs('admin.sellers.*')">{{ __('Verifikasi Seller') }}</x-responsive-nav-link>
            ...
            @if(Auth::check() && Auth::user()->role == 'seller' && Auth::user()->store_status == 'approved')
                <x-responsive-nav-link :href="route('seller.dashboard')" :active="request()->routeIs('seller.dashboard')">{{ __('Toko Saya') }}</x-responsive-nav-link>
                <x-responsive-nav-link :href="route('seller.products.index')" :active="request()->routeIs('seller.products.*')">{{ __('Produk Saya') }}</x-responsive-nav-link>
                <x-responsive-nav-link :href="route('seller.orders.index')" :active="request()->routeIs('seller.orders.*')">{{ __('Pesanan Masuk') }}</x-responsive-nav-link>
            @elseif(Auth::check() && Auth::user()->role == 'buyer')
                <x-responsive-nav-link :href="route('cart.index')" :active="request()->routeIs('cart.index')">{{ __('Keranjang Belanja') }}</x-responsive-nav-link>
                <x-responsive-nav-link :href="route('orders.index')" :active="request()->routeIs('orders.*')">{{ __('Pesanan Saya') }}</x-responsive-nav-link>
                <x-responsive-nav-link :href="route('favorite.index')" :active="request()->routeIs('favorite.index')">{{ __('Favorit Saya') }}</x-responsive-nav-link>
            @endif
        </div>
        ...
        <div class="pt-4 pb-1 border-t border-gray-200 bg-gray-50">
            ...
            @else
                <div class="px-4 pb-4 space-y-3">
                    <a href="{{ route('login') }}" class="block w-full text-center py-3 text-gray-700 bg-white border border-gray-300 rounded-xl font-semibold shadow-sm">Masuk</a>
                    <a href="{{ route('register') }}" class="block w-full text-center py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-xl font-semibold shadow-lg">Daftar Sekarang</a>
                </div>
            @endauth
        </div>
    </div>
